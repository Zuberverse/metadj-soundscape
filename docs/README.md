...
- Current production pod `<YOUR_RUNPOD_ID>` reports `ndi_available=false` and `spout_available=false`; external input toggles require a Scope runtime that exposes those capabilities.
...
- Current production hardware profile is RunPod pod `<YOUR_RUNPOD_ID>` on `RTX PRO 6000` (`96GB` VRAM), with `RTX 5090` still feasible for default/lower tiers.
